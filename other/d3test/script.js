...
function loadDataFromURL() {
    ...
    if (data) {
        ...
        if(data == 'dump') {
            fetch('https://api.robsweb.site/inspo/dump')
            .then(response => response.json())
            .then(data => {
...
d3.select(document).on("keydown", function (d) {
    if (selectedNode) {
        ...
        if (d.key === ']') {
            ...
            t++;
            addNode(newNodeData, selectedNode);
        }
    }
...
// addNode({
//     'name': 'Introduction',
//     'html': 'CTRL + click to select a node<br>Press ] to add a new input node.',
//     'children': [{
//         'name': 'Example',
//     }]
// })